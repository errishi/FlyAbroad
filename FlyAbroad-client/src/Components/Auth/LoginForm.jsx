...
const LoginForm = ({ setCurrentAuth = () => {} }) => {
    const [login, setLogin] = useState(false); // Note: true = Signup view, false = Login view
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            // FIXED: Changed response.data to data (Fetch API parsing)
            if (data.success) {
                navigate('/');
                setUser(data.user);
            ...
            // Close the login modal and redirect to home page
            setIsClosing(true);
            setTimeout(() => {
                setCurrentAuth(false);
                navigate('/');
            ...
            >
                ...
                <div className='relative flex w-full flex-col justify-center min-h-0 overflow-auto lg:overflow-visible bg-white px-4 py-6 sm:px-8 sm:py-8 lg:w-[48%] lg:px-14 lg:py-10 pr-4 lg:pr-0'>
                    ...
                    <div className='mx-auto w-full max-w-105'>
                        {login ? (
                            <h1 className='mb-6 text-center text-2xl font-extrabold text-[#0f3d46] sm:text-[34px] lg:text-left'>Create an account</h1>
                        ) : (
                            <h1 className='mb-6 text-center text-2xl font-extrabold text-[#0f3d46] sm:text-[34px] lg:text-left'>Log In</h1>
                        )}

                        <form className='flex flex-col gap-4 sm:gap-5' onSubmit={login ? handleSignup : handleLogin}>
                            {login ? (
                                <TextField
                                    fullWidth
                                    id='outlined-name'
                            ...
                            <div className='-mt-1 flex justify-end'>
                                <button
                                    type='button'
                                    onClick={() => { setIsClosing(true); navigate('/forget-password'); }}
                                    className='text-sm font-semibold text-[#0B7077] transition hover:text-[#FD661F]'
                                >
                                    Forgot Password?
                                </button>
                            </div>

                            {login ? <LoginButton name={"Sign Up"} disabled={loading} /> : <LoginButton name={"Login"} disabled={loading} />}
                            ...
                            <div className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
                                <button type='button' onClick={() => handleOAuthRedirect('google')} disabled={loading} className='flex h-12 items-center justify-center gap-2 rounded-full border border-gray-200 bg-white px-4 text-sm font-semibold text-gray-700 transition hover:border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed'>
                                    <GoogleIcon fontSize='small' />
                                    Google
                                </button>
                                <button type='button' onClick={() => handleOAuthRedirect('linkedin')} disabled={loading} className='flex h-12 items-center justify-center gap-2 rounded-full border border-gray-200 bg-white px-4 text-sm font-semibold text-gray-700 transition hover:border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed'>
                            ...
                            <div className='flex items-center justify-center gap-2 pt-2 text-sm font-medium text-gray-600'>
                                {login ? (
                                    <>
                                        ...
                                        <button type='button' onClick={() => { setLogin(!login); setError(''); }} disabled={loading} className='font-semibold cursor-pointer text-[#0B7077] hover:underline disabled:opacity-50 disabled:cursor-not-allowed'>Login</button>
                                    </>
                                ) : (
                                    <>
                                        <span>Don't have an account?</span>
                                        <button type='button' onClick={() => { setLogin(!login); setError(''); }} disabled={loading} className='font-semibold cursor-pointer text-[#0B7077] hover:underline disabled:opacity-50 disabled:cursor-not-allowed'>Sign Up</button>
                                    </>
                                )}
                            </div>
                        </form>
